...
<html lang="es">
  <head>
    ...
    <title> Restablecer contraseña | LeagueOfSigns </title>
...
<?php
   $conexion = conexion();
   $query = " SELECT * FROM tblreseteopass WHERE token = '$token' ";
   $resultado = $conexion->query($sql);
   if( $resultado->num_rows > 0 ){
      $usuario = $resultado->fetch_assoc();
      if( sha1( $usuario['nombre_usu'] === $vnombre_usu ) ){
         if( $password1 === $password2 ){
            $query = "UPDATE users SET password = '".sha1($password1)."' WHERE nombre_usu = ".$usuario['nombre_usu'];
            $resultado = $conexion->query($sql);
            if($resultado){
...
<?php
    ...
    //HACER RESTABLECERPW CON FORMULARIO PARA restablecer contraseña!
   header('Location:restablecerPW.html');
}
?>